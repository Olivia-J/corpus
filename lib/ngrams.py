...
def Ngrams(query,startYear=1950,endYear=1999):
    if isinstance(query,basestring):
        url, urlquery, df=getNgrams(query,'eng_2012',startYear,endYear,3,True)
        freq=0
        flag = False
        for col in df.columns:
            flag = flag or col.count(' (All)')
        ...
        for row in df.iterrows():
            try:
                freq+=row[1].values[len(row[1])-1]
            except:
                pass
        return freq/(endYear-startYear+1)
    else:
         squery=','.join(query)
         url, urlquery, df=getNgrams(squery,'eng_2012',startYear,endYear,3,True)
         query_alphabet=[]
         freq_dict={}
         for qu in query:
            if qu+' (All)' in df.columns:
                qu_g=qu+' (All)'
            elif qu in df.columns:
                qu_g=qu
            elif qu.capitalize() in df.columns:
                qu_g=qu.capitalize()
            elif qu.lower() in df.columns:
                qu_g=qu.lower()
            elif qu.upper() in df.columns:
                qu_g=qu.upper()
            elif qu.replace('-',' - ') in df.columns:
                qu_g=qu.replace('-',' - ')
            else: 
                url, urlquery, df=getNgrams(qu,'eng_2012',startYear,endYear,3,True)
                freq=0
#                print df.columns
                flag = False
                for col in df.columns:
                ...
                for row in df.iterrows():
                    try:
                        ...
                        pass
                freq=freq/(endYear-startYear+1)
                try:
                 freq_dict.update({qu:freq})
                except:
                 pass
...
#################################
         ...
         for col in df.columns:
             if col != 'year':
                 ww=col.replace(' - ','-')
                 ww=col.replace(' ','')
                 query_alphabet.append(ww)
#         print query_alphabet
#         print df
         freq=[]
         for row in df.iterrows():
             try:
                 ...
                 freq=row[1].values[1:]
         freq=numpy.array(freq)/(endYear-startYear+1)
         freq_dict={}
         
         for i in range(len(freq)):
             freq_dict.update({query_alphabet[i]:freq[i]})
         return freq_dict
