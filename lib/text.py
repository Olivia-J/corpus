...
class ConcordanceIndex(object):
    ...
    def freq(self):
        ...
        for word in self._offsets:
            freq.update({word:len(self._offsets[word])})
        return freq
...
class ExampleIndex(object):
    ...
    a given word occurs in a document.
    """
    def __init__(self, sentences, lemma_sentences, key=lambda x:x):
        """
        Construct a new concordance index.
        ...
        """The document (list of tokens) that this concordance index
           was created from."""
        self._lemma_sentences = lemma_sentences
        ...
        # Initialize the index (self._offsets)
        for index, sen in enumerate(tokens):
            for word in sen:
             word = self._key(word)
             self._offsets[word].append(index)
...
class Text(object):
    ...
    def concordance(self, word, width=79, lines=25):
        ...
        if '_concordance_index' not in self.__dict__:
            ...
            self._concordance_index = ConcordanceIndex(self.tokens,key=lambda s:s)
        self._concordance_index.print_concordance(Lemmatizer().lemmatize(word), width, lines)
    def example(self, word):
        # use to find sentence examples of a word
        if '_example_index' not in self.__dict__:
            #print("Building index...")
                self._example_index=ExampleIndex(self.sentences, self.lemma_sentences, key=lambda x:x)
        return self._example_index.sentenceslist(Lemmatizer().lemmatize(word))
...
#        words = self._word_context_index.similar_words(word, num)
    ...
    def vocabulary(self):
        ...
        if '_vocabulary' not in self.__dict__:
         fd=FreqDist(self)
         for word in fd:
            if len(word)>2 and (word not in ignored_words) and word[0].isalpha() and word[len(word)-1].isalpha() and (not '-' in word):
                if (not word[0].isupper()) and (a.check(word) or b.check(word)):
                    vocab.update({word:fd[word]})
...
def lemma(passage):
    try:
     ...
     tokens = nltk.word_tokenize(passage.decode('utf-8'))
    lemma_passage=''
    for word in tokens:
...
#        if (not (a.check(word) or b.check(word)) or word in names):
#            return word        
#    try:
#        y=word.lower()
#        y=Lemmatizer().lemmatize(y, pos='v')
#        y=Lemmatizer().lemmatize(y, pos='a')
#        y=Lemmatizer().lemmatize(y, pos='r')
#        y=Lemmatizer().lemmatize(y, pos='n')
#        return y
#    except:
